    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        if not is_grpo:
            if args.lazy_tokenize:
                if args.custom_data_aug:
                    if args.dynamic_ocr:
                        from swift.llm import Compose, RandomOCROrVQA
                        data_aug_train = Compose([RandomOCROrVQA(mode0_prob=args.mode0_prob, num_img_to_ocr=args.num_img_to_ocr, is_train=True)])
                        data_aug_val = Compose([RandomOCROrVQA(mode0_prob=args.mode0_prob, num_img_to_ocr=args.num_img_to_ocr, is_train=False)])
                train_dataset = LazyLLMDataset(
                    train_dataset, template.encode, strict=args.strict, random_state=args.data_seed, data_aug=data_aug_train)
                if val_dataset is not None and not args.predict_with_generate:
                    val_dataset = LazyLLMDataset(
                        val_dataset, template.encode, strict=args.strict, random_state=args.data_seed, data_aug=data_aug_val)
            else:
            ...
            if args.custom_data_aug:
                logger.info("After data_aug")
                inputs = train_dataset[0] if hasattr(train_dataset, '__len__') else next(iter(train_dataset))
                template.print_inputs(inputs, tokenizer_kwargs=inputs.pop('tokenizer_kwargs', None) or {})
                logger.info("Eval data")
                if val_dataset is not None:
                    inputs = val_dataset[0] if hasattr(train_dataset, '__len__') else next(iter(train_dataset))
                    template.print_inputs(inputs, tokenizer_kwargs=inputs.pop('tokenizer_kwargs', None) or {})
